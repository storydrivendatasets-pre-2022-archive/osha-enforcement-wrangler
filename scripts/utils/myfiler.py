...
def get_latest_snapshot_dir():
    ...
    datadirs = [
        p
        for p in SNAPSHOTS_DATA_DIR.glob("*/")
        if p.is_dir() and re.search(r"\d{4}-\d{2}-\d{2}", str(p))
    ]

    if not datadirs:
        raise ValueError(
            f"Could not find any valid collected zip directories in {SNAPSHOTS_DATA_DIR}"
        )
